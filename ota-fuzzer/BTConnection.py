    ...
    def _hciEventHandler(self, h4_record):
        ...
        if event == 3:
            ...
            if status == 0:
                ...
                self.handle = handle
                log.info("Connection to %s complete", binascii.hexlify(
                    self.remote_addr).decode("utf-8"))
        ...
        # link key request
            ...
            self.core.sendHciCommand(
                0x040b, bd_addr + bytes.fromhex("0d2017c7f90a78cefeeed32210e6519a"))
            return
        ...
        # link key notification
            ...
            log.info("Got Link Key notification from %s, key: %s",
                     bd_addr, binascii.hexlify(link_key).decode("utf-8"))
        ...
        # everything from le lands here...
            ...
            if le_event_type == 0x0a:
                ...
                self.core.sendHciCommand(
                    0x2012, bytes.fromhex("00") + self.remote_addr[::-1])
